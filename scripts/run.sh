...
NETWORK_OPS="-Dsun.net.inetaddr.ttl=30"
...
if [[ -z $(ps ax | egrep "$PROJECT_NAME.*jar|shadowJar" | grep 'grep' -v) ]]
    ...
    if [ "$systemMemory" -gt 2000000 ]
      then
         nohup java $SERVER_OPS $GC_OPS $WEB_SERVER_OPS $NETWORK_OPS $JAR_OP MARKETSTEM MARKET_DATA_CACHE AGGREGATE_TICKER MARKETSTEM_HTTP >>console.out 2>&1 &
      else
         nohup java $SERVER_OPS $GC_OPS $WEB_SERVER_OPS $NETWORK_OPS $JAR_OP MARKETSTEM MARKET_DATA_CACHE MARKETSTEM_HTTP >>console.out 2>&1 &
    fi
fi
